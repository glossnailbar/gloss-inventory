...
const changeSchema = zod_1.z.object({
    local_id: zod_1.z.string(),
    table: zod_1.z.enum(['products', 'categories', 'vendors', 'locations', 'inventory_levels']),
    operation: zod_1.z.enum(['create', 'update', 'delete']),
    data: zod_1.z.record(zod_1.z.any()),
    client_timestamp: zod_1.z.string(),
    client_version: zod_1.z.number(),
});
const pushRequestSchema = zod_1.z.object({
    ...
    try {
        ...
        for (const change of changes) {
            ...
            try {
                ...
                const existingResult = await client.query(`SELECT sync_version, updated_at FROM ${change.table} WHERE local_id = $1`, [change.local_id]);
                ...
                let filteredData = Object.fromEntries(Object.entries(change.data).filter(([key]) => !reservedColumns.includes(key)));
                ...
                for (const fkCol of foreignKeyColumns) {
                    ...
                    if (filteredData[fkCol] && typeof filteredData[fkCol] === 'string') {
                        ...
                        let refTable = '';
                        if (fkCol === 'category_id')
                            refTable = 'categories';
                        else if (fkCol === 'vendor_id')
                            refTable = 'vendors';
                        else if (fkCol === 'location_id')
                            refTable = 'locations';
                        ...
                        if (refTable) {
                            ...
                            const fkResult = await client.query(`SELECT id FROM ${refTable} WHERE ${lookupColumn} = $1 AND organization_id = $2`, [filteredData[fkCol], organization_id]);
             ...
             RETURNING id`;
                    ...
                    result = await client.query(`UPDATE ${change.table}
             SET ${setClause}, updated_at = NOW()
             WHERE local_id = $1 AND organization_id = $2
             RETURNING id`, [change.local_id, organization_id, ...values, change.client_version]);
                }
                else if (change.operation === 'delete') {
                    result = await client.query(`UPDATE ${change.table}
             SET deleted_at = NOW(), sync_version = $3, updated_at = NOW()
             WHERE local_id = $1 AND organization_id = $2
             RETURNING id`, [change.local_id, organization_id, change.client_version]);
                }
                accepted.push({
                    local_id: change.local_id,
        ...
        const sequenceResult = await client.query('SELECT nextval(\'sync_sequence\') as sequence');
...
router.get('/pull', async (req, res) => {
    try {
      ...
      SELECT * FROM (
        ...
        SELECT 'inventory_levels' as table_name, il.id, COALESCE(il.local_id, il.id::text) as local_id, il.sync_version as server_sequence, il.created_at,
...
router.post('/setup-inventory', async (req, res) => {
    try {
        ...
        try {
            ...
            const locationCheck = await client.query('SELECT id FROM locations WHERE organization_id = $1 LIMIT 1', [organization_id]);
            if (locationCheck.rows.length === 0) {
                // Create default location
                await client.query(`INSERT INTO locations (id, local_id, organization_id, name, is_active, sync_version, created_at, updated_at)
           VALUES ($1, $2, $3, $4, true, 1, NOW(), NOW())
           ON CONFLICT (id) DO NOTHING`, [locationId, 'default-loc-' + organization_id.slice(0, 8), organization_id, 'Main']);
            }
            else {
                locationId = locationCheck.rows[0].id;
            }
            // Create inventory_levels for all products that don't have them
            const result = await client.query(`INSERT INTO inventory_levels (
        ...
        RETURNING id`, [organization_id, locationId]);
            await client.query('COMMIT');
            res.json({
                success: true,
